...
// Strip markdown formatting for parsing
...
function parseMarkdown(content) {
  ...
  for (const line of lines) {
    ...
    // Parse header info - check for contains since format may vary
    ...
    // Parse account fields (- Key: Value)
    else if (currentAccount && line.trim().startsWith('- ')) {
      const fieldLine = line.trim().replace('- ', '');
